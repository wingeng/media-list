...
lines = File.open("/tmp/t").readlines
...
lines.each {|x|
  ...
  insert_stmt = 'insert or replace into isbn(timestamp, isbn, used_price, in_media, title, author, date_of_publication, binding) ' + 
  "VALUES('#{timestamp}', '#{isbn}', '#{used_price}', '#{in_media}', '#{title}', '#{author}', '#{date_of_publication}', '#{binding}');"
